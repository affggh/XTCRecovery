...
if exist "%search_dir%\rawprogram1.xml" (
    "%local_dir%\xtcfh_loader.exe" --port=\\.\%COMPORT% --memoryname=EMMC --search_path="%search_dir%" --sendxml=rawprogram0.xml,rawprogram1.xml,rawprogram2.xml,patch0.xml --noprompt --reset --simulate  || exit /b 1
) else (
    "%local_dir%\xtcfh_loader.exe" --port=\\.\%COMPORT% --memoryname=EMMC --search_path="%search_dir%" --sendxml=rawprogram0.xml,patch0.xml --noprompt --reset --simulate  || exit /b 1
)
echo Done!
